...
interface StoredProgress { visited: string[]; answered: string[]; completed: string[]; lastChapter: string; }
const EMPTY: StoredProgress = { visited: [], answered: [], completed: [], lastChapter: '1-1' };
function readProgress(): StoredProgress {
  try {
    ...
    if (legacy) {
      ...
      return { visited: (old.visited ?? []).map((n: number) => `1-${n}`), completed: (old.completed ?? []).map((n: number) => `1-${n}`), answered: (old.answered ?? []).map((id: string) => `1-${id}`), lastChapter: `1-${old.lastChapter ?? 1}` };
    }
...
export function useProgress() {
  ...
  const markAnswered = useCallback((id: string) => update((current) => ({ ...current, answered: current.answered.includes(id) ? current.answered : [...current.answered, id] })), [update]);
